namespace BloodAndBittersteel
{
    public static class Globals
    {
        //@TODO 23.07 BaBKingdomDecisionPermissionModel.IsWarDecisionAllowedBetweenKingdoms needs to be updated
        public const string IronbornCultureId = "vlandia";
        public const string WildlingsCultureId = "empire";

        public const string NightsWatchKingdomStringId = "vlandia";
        public const string SkagosKingdomId = "sturgia";
        public const string BlackfyreRebellionKingdomStringId = "bab_blackfyre_rebels";
        public const string CrownlandsKingdomStringId = "Crownlands";
        public const string DorneKingdomId = "Dorne";
        public const string NewDornishKingdomId = "bab_dornish_rebels";
        public const string FreeFolkKingdomStringId = "bab_free_folk";
        public const string KhuzaitKingdomId = "khuzait"; //@TODO needs to be edited
        public const string EmpireStringId = "empire";
        public const string ValeKingdomId = "Vale";
        public const string WesterlandsKingdomId = "Westerlands";
        public const string NorthKingdomId = "North";
        public const string IronbornKingdomId = "Ironborn";
        public const string ReachKingdomId = "Reach";
        public const string StormlandsKingdomId = "Stormlands";
        public const string RiverlandsKingdomId = "Riverlands";
    }
}