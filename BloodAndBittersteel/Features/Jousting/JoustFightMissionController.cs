    ...
    public class JoustFightMissionController : MissionLogic, ITournamentGameBehavior
    {
        ...
        public void PrepareForMatch()
        {
            ...
            foreach (TournamentTeam tournamentTeam in _match.Teams)
            {
                ...
                {
                    participantWeaponEquipmentList[num][EquipmentIndex.Horse] = new(MBObjectManager.Instance.GetObject<ItemObject>("aserai_horse_tournament"));
        ...
        public void OnMatchResultsReady()
        {
            ...
            if (_match.IsPlayerWinner())
            {
                if (_isLastRound)
                {
                    MBInformationManager.AddQuickInformation(new TextObject("Round is over, you survived the final round of the tournament."));
                    return;
                }
                else
                {
                    MBInformationManager.AddQuickInformation(new TextObject("Round is over, you are qualified for the next stage of the tournament."));
                    return;
                }
            }
            else
            {
                MBInformationManager.AddQuickInformation(new TextObject("Round is over, you are disqualified from the tournament."));
                return;
            }
        }
